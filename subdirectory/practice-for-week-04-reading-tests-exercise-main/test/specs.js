...
// the outer function isFive(num) checks to see if a number is a Five or not,
// returning true if it is a five and otherwise false
// if number is not type of Number  return false
...
// outer function isOdd(number) checks if number is odd
// if number is odd return true,
// if number is even return false,
// and number not a type of Number return an errror
...
// outer function myRange() should return an array of numbers between min and max
// if min is greater than max return a empty array
// if step is not provided using the default value step = 1
// otherwise if step is provided use the provided step
